...
- [ ] P2 — R-137: Normalize the control geometry the panel renders
  ...
  Found 2026-08-25 while reading for this item, not fixed: the test named "one focus treatment, defined once and used everywhere" only scans for `outline: 0` written on a `:focus` selector line. `.kf-command-head input` sets `outline: 0` in its **base** rule and gets only a box-shadow on `:focus`, so it slips past a test whose name says it cannot. The same shape covers `.kf-text`, `.kf-textarea`, `.kf-select` and both multi-stream inputs, which are exactly the controls the forced-colors block has to name individually. Not a contrast failure outside forced colors, where the shadow is a visible indicator, but it is the second focus treatment this item exists to remove, and the test should catch it before the CSS does.
  Complexity: M
