...
- [ ] P1: R-165: Fix the reward auto-claim check the live gate fails on every run
  Why: `a ready reward is claimed once and then sleeps to the nightly rollover` fails identically at 630662b and at c1fffba, so it is a standing failure rather than a regression and it is failing on the one path that takes an action on the user's account. Measured 2026-09-05: the run reports `claimed on pass 1 of 8: clicks=1, final action=Claim, pending=1788624763185/1788624763572, stored claims=0, next check at hour 12; second pass clicked 0`. The click happened and the final action stayed `Claim` rather than becoming `Share`, so success was never confirmed, and `stored claims=0` means the record the feature uses to avoid re-claiming was not written. The next-check hour also lands at 12 rather than the 20:00 rollover the neighbouring passing check asserts.
  Evidence: `npm run verify:extension` at 630662b and at c1fffba, both 103/105 with this check among the two failures; src/runtime.js reward claim state machine (`runRewardClaim`, `confirmRewardClaim`, `failRewardClaim`, `rewardRecord`); the passing neighbours `a reward already collected by hand is observed without inventing an automatic claim` and `the countdown Kick shows sets the next look, not the fallback interval`
  Touches: src/runtime.js reward claim and scheduling, src/core.mjs reward record normalization, scripts/verify-extension.mjs reward journey, test/reward-live.test.js
  Acceptance: A run that claims a ready reward records the claim, reports a final action of Share, and sets the next check to the printed rollover; a run against an already-claimed reward still clicks nothing; the check passes twice in a row against live Kick; and if the observed final action is not Share the run reports that Kick did not confirm rather than counting a claim.
  Complexity: M

- [ ] P2: R-166: Stop the live chat checks failing on timing rather than on behaviour
  Why: Two of the live gate's chat checks fail non-deterministically. At 630662b the failure was `a 300-message burst with every chat comfort switch on stays inside the apply budget`; at c1fffba, minutes later and with no chat code changed between them, that one passed and `scrolling chat up enters the paused state, and Resume leaves it` failed instead. A gate that names a different check each run cannot distinguish a real regression from a busy channel, which is what it exists to do.
  Evidence: two `npm run verify:extension` runs on 2026-09-05, one per commit, each 103/105 with a different second failure; both checks depend on a live channel producing messages at an unspecified rate
  Touches: scripts/verify-extension.mjs chat pause and apply-budget checks, scripts/live-contract.mjs
  Acceptance: Each of the two checks either establishes its own precondition (a chat producing enough messages to measure) and reports a skip naming what was missing when it cannot, or measures against a seeded transcript rather than whatever the live channel happens to be doing; ten consecutive runs produce no failure attributable to message rate; a genuine regression in either behaviour still fails.
  Complexity: M
