...
- [ ] P2 — R-49, say when a VOD expires
  ...
  Evidence: help.kick.com/en/articles/7112432 (retention tiers). **The join is solved — measured on live Kick 2026-08-18.** The endpoint is `GET https://web.kick.com/api/v1/channels/{channel_id}/videos` (note: **web.kick.com v1, keyed by channel *id***, not the `kick.com/api/v2/channels/{slug}/videos` a previous pass tested). It answers 200 with `{ data: [...], message }`, 25 entries, and each entry's **`id` is exactly the v7 UUID in the page URL** — `id=01a01256-da48-7d57-8bf2-0ac2745d698d` for `/xqc/videos/01a01256-da48-7d57-8bf2-0ac2745d698d`. Each entry also carries `start_time` and `end_time` as **zone-declared ISO** (`2026-08-18T00:47:57Z`), which `parseKickTimestamp` already passes through untouched, plus `duration`, `status`, `title`, `channel.slug` and `channel.id`.
  **Two traps, both measured, do not repeat them.** (1) The entry's `tier` field is **not** the retention tier: xQc's channel reads `verified: true` while every one of its VOD entries reads `tier: "unverified"`. Take the tier from the channel read's `verified`, never from `tier`. The observed window corroborates the channel flag — xQc's 25 entries span 2026-07-19T23:15:15Z to 2026-08-18T00:47:57Z, which is 30 days, the *verified* retention. (2) There is **no single-video read**: `web.kick.com/api/v1/{videos,video,streams}/{uuid}` and `kick.com/api/v1/videos/{uuid}` are all 404, and `web.kick.com/api/v1/stream/{uuid}/playback` is 404 for a finished VOD even though the page itself calls it. So the list is the only path, and a VOD outside the 25 most recent cannot be resolved — that must be a silence, not a guess.
  Also corrected: a VOD page **does** carry the recording date, contrary to the earlier note. The served markup contains `2026-08-18T00:47:57` and the page itself requests `chat/{chatroom}/history?start_time=2026-08-18T00:47:57Z`. It is only *linked data* (`VideoObject`) that is absent. The API read above is still the right source — it is structured, and the markup position is not a contract.
  Touches: `src/api.mjs` (`endpoints.channelVideos`, `verified` on `normalizeChannel`, a `normalizeChannelVideos`), `src/live.mjs`, `src/runtime.js`, `src/core.mjs`, `test/api.test.js`, `test/core.test.js`
  Acceptance: A VOD page shows time remaining before Kick's retention window closes, computed with the same UTC-normalising parse the uptime chip uses and the tier taken from the channel's `verified`; nothing is downloaded or written; the surface says nothing at all when the entry is not in the list, when the channel read failed, or when the tier cannot be determined, rather than guessing 7 or 30. The channel VOD *list* page stays out of reach separately — its cards carry only localized relative text ("14 hours ago") with no `<time datetime>`, confirmed again 2026-08-18.
  Complexity: M
