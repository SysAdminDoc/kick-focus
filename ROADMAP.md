...
- [ ] P2 — R-83 — Show live duration on discovery cards from data Kick already sent
  ...
  Implementation note (2026-08-21): the passive fetch/XHR observer, bounded normalizer, exact LIVE-marker join, compact clock, cleanup, and browser geometry gate are implemented. Offline verification passes 357/357. The checkbox remains open because the headed Kick gate has stalled before completing the new browser probe in repeated runs.
...
- [ ] P2 — R-79 — Add `layout.chat = 'left'` without breaking the theater separator
  ...
  Complexity: L
  Implementation note (2026-08-21): Left is a normalized, persisted Layout option with Spanish and Portuguese labels. The chat owner moves before the player, its split reverses so the separator stays on the player-facing edge, and shared drag math grows the column in the correct direction on either side while preserving the 320–520 px bounds. The live probe now selects Left through the UI, checks persistence, Theater containment, both rendered chat layers, drag state, and reversal to Right. Offline verification passes 360/360 with 185 artifact checks and 80 red probes. The checkbox remains open because the headed Kick gate has stalled before completing the new browser probe in repeated runs.
...
- [ ] P3 — R-87 — Following-rail hover preview from Kick’s own thumbnail
  ...
  Implementation note (2026-08-21): followed-channel controls are tagged only when Kick already placed an image in the row. Hover and focus open one fixed, viewport-clamped figure, Escape closes it, and the active row owns the tooltip description. System or in-app Reduced Motion draws the loaded image into a static canvas. The feature contains no network read and adds no permission. Offline verification passes 362/362 with 187 artifact checks and 80 red probes. The live gate now exercises native or synthetic rail geometry plus emulated reduced motion, but the headed Kick session stalled before its first verdict, so the checkbox remains open.
