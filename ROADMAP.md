...
- [ ] P2: R-166: Stop the live chat checks failing on timing rather than on behaviour
  ...
  Note 2026-09-05: a third instance, and a third different check — a run minutes later failed only on `every live followed channel expands past Kick’s five-row sidebar preview`, which depends on what Kick renders in the sidebar for an anonymous visitor at that moment. Three runs, three different single failures, no source change between them. The class is wider than the two chat checks named above.
  Shipped 2026-09-05, partially: the followed-channel expansion check now pokes the route between polls instead of waiting passively, and its window went from 3.5s to 8s. That one was betting the coalesced apply cycle happened to run twice inside the window, which on a busy page it sometimes did not. Two 105/105 runs since. Still open for the two chat checks, whose failure mode is different — both depend on what a live channel is doing, and the apply-budget one asserts an absolute 120 ms average that a loaded machine can miss without anything being wrong. Neither is fixed by an active wait, and the ten-consecutive-run bar in the acceptance was not attempted.
  Complexity: M
