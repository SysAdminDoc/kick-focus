/* Kick Focus 1.48.0 — generated from src/. Edit the source, not this file. */
(() => {
'use strict';
...
function blocklistResponseHeader(headers, name) {
...
function assertBlocklistJsonResponse(response, expectedUrl, maxBytes, headers = response?.headers) {
...
async function readBoundedBlocklistBody(response, maxBytes) {
...
const STICKER_PREFERENCES_SCHEMA = 11;
function stampStickerCommit(value, writer, sequence) {
...
function stickerCommitStamp(value) {
...
function sameStickerCommit(left, right) {
...
function selectEmoteDockKeys({ favorites = [], recent = [], available = [], mode = 'mixed', limit = 6 } = {}) {
...
function stickerSearchRank(sticker, query, customGroup = '') {
...
const LIBRARY_SEED_BYTES = 33_000;
...
function planLibraryPersist(value, {
...
return Boolean(chatWindow && !chatWindow.closed);
  ...
  const POPOUT_CSS = `:root{color-scheme:dark}body{margin:0;display:flex;flex-direction:column;height:100vh;background:var(--kf-panel,#0b100d);color:var(--kf-text,#f5f8f6);font:12px/1.4 Inter,ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif}p{margin:0;padding:6px 8px;background:var(--kf-panel-raised,#111713);border-bottom:1px solid var(--kf-border,#202a23);color:var(--kf-text-muted,#aab4ae);font-size:11px}iframe{flex:1 1 auto;width:100%;border:0}`;
...
if (result.ok) {
try { stickerSyncChannel()?.postMessage({ type: 'emotes-changed' }); } catch {   }
}
return result;
}
...
const TOOLTIP_CSS = `:host{position:fixed;z-index:2147483000;pointer-events:none;display:none;max-width:280px}:host([data-kf-open="true"]){display:block}:host([data-kf-anchored="true"]){inset:auto;margin:0;border:0;padding:0;background:transparent;overflow:visible;color:inherit;position-area:block-start span-inline-end;position-try-fallbacks:flip-block,flip-inline,flip-block flip-inline}.card{padding:8px 10px;border:1px solid var(--kf-border-strong,#38463d);border-radius:var(--kf-radius,7px);background:var(--kf-panel-raised,#111713);color:var(--kf-text,#f5f8f6);box-shadow:0 10px 28px rgba(0,0,0,.45);font:12px/1.45 Inter,ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif}.card div{white-space:normal;overflow-wrap:anywhere}.card div:first-child{font-weight:700}.card div + div{color:var(--kf-text-muted,#aab4ae)}.card div[data-warn="true"]{color:var(--kf-warning,#f6b943)}`;
...
const UI_CSS = `:host{color-scheme:dark;--accent:var(--kf-accent,#7cff2b);--accent-rgb:var(--kf-accent-rgb,124,255,43);--surface-0:var(--kf-canvas,#070a08);--surface-1:var(--kf-panel,#0b100d);--surface-2:var(--kf-panel-raised,#111713);--surface-3:var(--kf-panel-high,#18201b);--surface-inset:var(--kf-surface-inset,#070b08);--surface-hover:var(--kf-surface-hover,#171f1a);--surface-selected:var(--kf-surface-selected,#111d14);--surface-danger:#2a1416;--border:var(--kf-border,#202a23);--border-subtle:color-mix(in srgb,var(--border) 68%,transparent);--border-control:color-mix(in srgb,var(--border-strong) 78%,var(--border));--border-strong:var(--kf-border-strong,#38463d);--text:var(--kf-text,#f5f8f6);--text-secondary:var(--kf-text-secondary,#d0d7d3);--muted:var(--kf-text-muted,#aab4ae);--subtle:color-mix(in srgb,var(--muted) 72%,var(--surface-0));--on-accent:var(--kf-on-accent,#071004);--danger:var(--kf-danger,#ff6258);--danger-text:#ffaaa4;--warning:var(--kf-warning,#f6b943);--success:var(--accent);--radius-none:0;--radius-xxs:4px;--radius-xs:4px;--radius-sm:6px;--radius-md:var(--kf-radius,8px);--radius-lg:12px;--radius:var(--kf-radius,8px);--control-height:40px;--control-height-small:32px;--focus-ring:var(--kf-focus-ring,3px solid var(--accent));--focus-offset:var(--kf-focus-offset,2px);--shadow-dialog:0 38px 110px rgba(0,0,0,.72),0 0 0 1px rgba(255,255,255,.015);--shadow-control:0 10px 28px rgba(0,0,0,.24);font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;font-size:14px;line-height:1.45}*,*::before,*::after{box-sizing:border-box}[hidden]{display:none!important}button,input,select,textarea{font:inherit}button{color:inherit}:is(button,input,select,textarea){transition:border-color 150ms ease,background-color 150ms ease,color 150ms ease,box-shadow 150ms ease,transform 150ms ease}::selection{background:rgba(var(--accent-rgb),.28);color:var(--text)}.kf-quick{position:fixed;left:18px;bottom:18px;z-index:2147483000;min-width:76px;height:var(--control-height);padding:0 16px;border:1px solid var(--border-control);border-radius:var(--radius-md);background:var(--surface-2);color:var(--text);box-shadow:0 14px 38px rgba(0,0,0,.5);cursor:pointer;font-size:12px;font-weight:800;letter-spacing:.06em;text-transform:uppercase}.kf-quick:hover{border-color:var(--accent);background:var(--surface-hover);color:var(--accent);transform:translateY(-1px)}.kf-quick:active{transform:translateY(0)}:host([data-kf-quick-avoid-composer="true"]) .kf-quick{bottom:126px}.kf-backdrop{position:fixed;inset:0;z-index:2147483200;display:grid;place-items:center;padding:22px;background:radial-gradient(circle at 50% 14%,rgba(var(--accent-rgb),.055),transparent 38%),rgba(2,3,3,.9)}.kf-settings{position:relative;zoom:var(--kf-interface-scale,1);width:min(1140px,calc((100vw - 44px) / var(--kf-interface-scale,1)));height:min(940px,calc((100vh - 44px) / var(--kf-interface-scale,1)));min-width:calc(860px / var(--kf-interface-scale,1));min-height:calc(640px / var(--kf-interface-scale,1));display:grid;grid-template-rows:76px minmax(0,1fr) 68px;overflow:hidden;border:1px solid var(--border);border-radius:var(--radius);background:var(--surface-1);box-shadow:var(--shadow-dialog);color:var(--text);font-size:14px}.kf-header{min-width:0;display:grid;grid-template-columns:minmax(0,1fr) auto auto;align-items:center;gap:18px;padding:0 20px;border-bottom:1px solid var(--border-subtle);background:var(--surface-2)}.kf-brand{display:flex;align-items:center;gap:9px;min-width:0;font-size:16px;font-weight:820;letter-spacing:-.02em}.kf-brand-mark{width:28px;height:28px;display:block;object-fit:contain}.kf-badge{padding:2px 6px;border:1px solid rgba(var(--accent-rgb),.68);border-radius:var(--radius-xs);color:var(--accent);font-size:9px;font-weight:900;letter-spacing:.06em;text-transform:uppercase}.kf-save{display:flex;align-items:center;color:var(--text-secondary);font-size:12px;font-weight:650}.kf-save::before{content:'';display:inline-block;width:8px;height:8px;margin-right:8px;border:1px solid var(--accent);border-radius:var(--radius-xxs);background:var(--accent)}.kf-save[data-error="true"]{color:var(--danger)}.kf-save[data-error="true"]::before{border-color:var(--danger);background:var(--danger)}.kf-icon-button{width:var(--control-height);height:var(--control-height);display:grid;place-items:center;border:1px solid transparent;border-radius:var(--radius-sm);background:transparent;color:var(--text);cursor:pointer;line-height:1}.kf-icon-button:hover{border-color:var(--border-control);background:var(--surface-hover)}.kf-icon-button:active{background:var(--surface-selected);transform:translateY(1px)}.kf-icon{width:18px;height:18px;display:block;fill:none;stroke:currentColor;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round}.kf-body{min-width:0;min-height:0;display:grid;grid-template-columns:232px minmax(0,1fr)}.kf-nav{padding:14px 10px;border-right:1px solid var(--border);background:var(--surface-0)}.kf-nav button{position:relative;width:100%;display:grid;grid-template-columns:24px minmax(0,1fr);align-items:center;gap:13px;min-height:56px;padding:0 14px;border:1px solid transparent;border-radius:var(--radius-md);background:transparent;text-align:left;cursor:pointer}.kf-nav button::before{content:'';position:absolute;inset:12px auto 12px 0;width:3px;border-radius:var(--radius-xxs);background:transparent}.kf-nav button:hover{border-color:var(--border-subtle);background:var(--surface-hover)}.kf-nav button:active{transform:translateY(1px)}.kf-nav button[aria-current="page"]::before{background:var(--accent);box-shadow:0 0 14px rgba(var(--accent-rgb),.35)}.kf-nav button[aria-current="page"]{border-color:rgba(var(--accent-rgb),.22);background:var(--surface-selected);color:var(--text)}.kf-nav .kf-icon{width:20px;height:20px;color:var(--text-secondary)}.kf-nav button[aria-current="page"] .kf-icon{color:var(--accent)}.kf-nav-copy{display:grid;gap:2px;min-width:0}.kf-nav strong{font-size:13px;font-weight:760}.kf-nav-copy>span:not(.kf-nav-earned){display:-webkit-box;overflow:hidden;color:var(--muted);font-size:11px;line-height:1.3;-webkit-box-orient:vertical;-webkit-line-clamp:2;white-space:normal}.kf-page{min-width:0;overflow-x:hidden;overflow-y:auto;padding:24px 34px 40px 36px;scrollbar-color:var(--border-control) transparent;scrollbar-width:thin}.kf-page:focus{outline:0}.kf-nav-search{padding:0 10px 10px}.kf-nav-search input{width:100%;min-height:var(--control-height-small);padding:6px 10px;border:1px solid var(--border);border-radius:var(--radius-sm);background:var(--surface-inset);color:var(--text);font:inherit;font-size:12px}.kf-nav-search input:focus-visible{outline:var(--focus-ring);outline-offset:var(--focus-offset)}.kf-search-results{display:flex;flex-direction:column;gap:8px;padding:12px 0}.kf-search-result{display:flex;align-items:center;justify-content:space-between;gap:16px;width:100%;min-height:58px;padding:10px 12px;border:1px solid var(--border-subtle);border-radius:var(--radius-sm);background:var(--surface-inset);color:var(--text);font:inherit;text-align:left;cursor:pointer}.kf-search-result:hover,.kf-search-result:focus-visible{border-color:var(--accent);background:var(--surface-inset)}.kf-search-result-copy{display:flex;flex-direction:column;gap:2px;min-width:0}.kf-search-result-copy strong{font-size:13px}.kf-search-result-copy small{color:var(--muted);font-size:11px;line-height:1.35}.kf-search-result-trail{display:inline-flex;flex:none;align-items:center;gap:9px;color:var(--muted)}.kf-search-result-trail .kf-icon{width:15px;height:15px}.kf-search-result-page{color:inherit;font-size:11px;text-transform:uppercase;letter-spacing:.06em}.kf-search-empty{max-width:680px;margin-top:12px;padding:18px;border:1px solid var(--border-subtle);border-radius:var(--radius-md);background:var(--surface-inset);color:var(--muted)}.kf-search-empty p{margin:0 0 6px}.kf-page-header{min-height:86px;display:flex;align-items:center;justify-content:space-between;gap:28px;padding-bottom:20px;border-bottom:1px solid var(--border)}.kf-page-header h2{margin:2px 0 5px;font-size:29px;line-height:1.06;letter-spacing:-.04em}.kf-page-header p{max-width:560px;margin:0;color:var(--muted);font-size:13px;line-height:1.45}.kf-eyebrow{display:block;color:var(--accent);font-size:10px;font-weight:850;letter-spacing:.12em;text-transform:uppercase}.kf-page-meta{display:grid;gap:3px;min-width:140px;text-align:right}.kf-page-meta span{color:var(--subtle);font-size:10px;font-weight:850;letter-spacing:.1em;text-transform:uppercase}.kf-page-meta strong{color:var(--text-secondary);font-size:12px;font-weight:740}.kf-page-meta-control{min-width:118px;justify-items:end}.kf-panel{border:0;border-radius:var(--radius-none);background:transparent;overflow:visible}.kf-row{min-height:74px;display:grid;grid-template-columns:minmax(230px,1fr) minmax(300px,auto);align-items:center;gap:26px;padding:12px 0;border-bottom:1px solid var(--border-subtle)}.kf-row h3{margin:0 0 4px;color:var(--text);font-size:13px;font-weight:780;letter-spacing:.01em}.kf-row p{max-width:420px;margin:0;color:var(--muted);font-size:12px;line-height:1.5}.kf-row-wide{grid-template-columns:minmax(220px,.82fr) minmax(360px,1.18fr)}.kf-control{min-width:300px;display:flex;justify-content:flex-end}.kf-segmented{display:inline-flex;border:1px solid var(--border-control);border-radius:var(--radius-md);overflow:hidden;background:var(--surface-inset);box-shadow:inset 0 1px rgba(255,255,255,.02)}.kf-segmented button{min-width:78px;height:var(--control-height);padding:0 13px;border:0;border-left:1px solid var(--border-control);background:transparent;color:var(--text-secondary);cursor:pointer;font-size:12px;font-weight:680}.kf-segmented button:first-child{border-left:0}.kf-segmented button:hover{background:var(--surface-hover);color:var(--text)}.kf-segmented button:active{background:var(--surface-selected)}.kf-segmented button[aria-pressed="true"]{background:rgba(var(--accent-rgb),.1);color:var(--text);box-shadow:inset 0 0 0 1px var(--accent)}.kf-hide-grid{display:grid;gap:14px;width:100%}.kf-hide-heading{display:block;margin-bottom:7px;color:var(--muted);font-size:10px;font-weight:850;letter-spacing:.1em;text-transform:uppercase}.kf-hide-chips{display:flex;flex-wrap:wrap;gap:6px}.kf-hide-chip{min-height:32px;padding:0 11px;border:1px solid var(--border-control);border-radius:var(--radius-md);background:var(--surface-inset);color:var(--text-secondary);cursor:pointer;font-size:12px;font-weight:680}.kf-hide-chip:hover{background:var(--surface-hover);color:var(--text)}.kf-hide-chip:active{background:var(--surface-selected)}.kf-hide-chip[aria-pressed="true"]{background:rgba(var(--accent-rgb),.1);color:var(--text);box-shadow:inset 0 0 0 1px var(--accent);text-decoration:line-through;text-decoration-color:var(--accent)}.kf-switch{width:58px;height:var(--control-height);position:relative;border:1px solid var(--border-control);border-radius:var(--radius-md);background:var(--surface-inset);color:var(--text-secondary);cursor:pointer;font-size:10px;font-weight:850;letter-spacing:.06em;text-transform:uppercase}.kf-switch:hover{border-color:var(--border-strong);background:var(--surface-hover)}.kf-switch[aria-checked="true"]{border-color:var(--accent);background:var(--accent);color:var(--on-accent);box-shadow:0 0 0 1px rgba(var(--accent-rgb),.12)}.kf-switch:disabled{opacity:.72;cursor:not-allowed}.kf-text::placeholder,.kf-textarea::placeholder{color:var(--muted);opacity:1}.kf-lock{display:inline-block;margin-left:7px;padding:2px 6px;border:1px solid rgba(var(--accent-rgb),.5);border-radius:var(--radius-xs);color:var(--accent);font-size:9px;font-weight:850;text-transform:uppercase}.kf-range{display:grid;grid-template-columns:48px minmax(220px,1fr) 48px;align-items:end;gap:10px;width:100%}.kf-range span{color:var(--muted);font-size:11px}.kf-range span:last-child{text-align:right}.kf-range-wrap{position:relative;display:grid;gap:4px}.kf-range output{justify-self:center;min-width:52px;padding:2px 7px;color:var(--text);text-align:center;font-size:10px;font-weight:750}.kf-range input{width:100%;height:16px;accent-color:var(--accent)}.kf-text,.kf-textarea{width:100%;min-height:var(--control-height);padding:9px 11px;border:1px solid var(--border-control);border-radius:var(--radius-md);background:var(--surface-inset);color:var(--text)}.kf-textarea{min-height:86px;resize:vertical}.kf-text:focus-visible,.kf-textarea:focus-visible{border-color:var(--accent)}.kf-select{min-width:118px;height:var(--control-height);padding:0 28px 0 10px;border:1px solid var(--border-control);border-radius:var(--radius-md);background:var(--surface-inset);color:var(--text);font-size:12px}.kf-select:hover{border-color:var(--border-strong)}.kf-select:focus-visible{border-color:var(--accent)}.kf-theme-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}.kf-swatch-grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:7px}.kf-preset-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.kf-preset-card{position:relative;display:grid;align-content:start;gap:4px;min-height:76px;padding:12px;border:1px solid var(--border);border-radius:var(--radius-md);background:var(--surface-inset);color:var(--text);text-align:left;cursor:pointer}.kf-preset-card::before{content:'';position:absolute;inset:0 auto 0 0;width:2px;background:rgba(var(--accent-rgb),.5);opacity:.55}.kf-preset-card:hover{border-color:var(--border-strong);background-color:var(--surface-hover);transform:translateY(-1px);box-shadow:var(--shadow-control)}.kf-preset-card:active{transform:translateY(0);box-shadow:none}.kf-preset-card span{color:var(--accent);font-size:9px;font-weight:850;letter-spacing:.09em;text-transform:uppercase}.kf-preset-card strong{font-size:12px}.kf-preset-card small{color:var(--muted);font-size:10px;line-height:1.4}.kf-theme-board{position:relative;min-height:126px;display:grid;align-content:start;gap:10px;padding:12px;border:1px solid var(--border);border-radius:var(--radius-md);background:var(--surface-inset);color:var(--text);cursor:pointer;text-align:left}.kf-theme-board:hover{border-color:var(--border-strong);transform:translateY(-2px);box-shadow:var(--shadow-control)}.kf-theme-board:active{transform:translateY(0);box-shadow:none}.kf-theme-board[aria-pressed="true"]{border-color:var(--accent);background:var(--surface-selected);box-shadow:inset 0 0 0 1px rgba(var(--accent-rgb),.15)}.kf-theme-board-top{display:flex;align-items:center;justify-content:space-between;gap:8px}.kf-theme-board-top>span:first-child{color:var(--subtle);font-size:8px;font-weight:850;letter-spacing:.1em;text-transform:uppercase}.kf-theme-selected{min-height:15px;color:var(--accent);font-size:8px;font-weight:850;letter-spacing:.08em;text-transform:uppercase}.kf-theme-tones{height:34px;display:grid;grid-template-columns:1.4fr 1fr .8fr;gap:3px;padding:4px;border:1px solid var(--theme-border);border-radius:var(--radius-sm);background:var(--theme-canvas)}.kf-theme-tones i{display:block;border-radius:var(--radius-xxs);background:var(--theme-panel)}.kf-theme-tones i:nth-child(2){background:var(--theme-raised)}.kf-theme-tones i:nth-child(3){background:var(--theme-high)}.kf-theme-board[data-value="studio"]{--theme-canvas:#080b09;--theme-panel:#0e130f;--theme-raised:#141a16;--theme-high:#1a221c;--theme-border:#46564b}.kf-theme-board[data-value="oled"]{--theme-canvas:#000;--theme-panel:#030404;--theme-raised:#080a09;--theme-high:#0e1110;--theme-border:#414b45}.kf-theme-board[data-value="slate"]{--theme-canvas:#0b0f14;--theme-panel:#111820;--theme-raised:#18222c;--theme-high:#202d39;--theme-border:#5a7084}.kf-theme-copy{display:grid;gap:2px}.kf-theme-copy strong{font-size:12px}.kf-theme-copy small{color:var(--muted);font-size:9px;line-height:1.35}.kf-accent-chip{min-width:0;min-height:52px;display:grid;grid-template-columns:18px minmax(0,1fr);align-items:center;gap:7px;padding:8px;border:1px solid var(--border);border-radius:var(--radius-md);background:var(--surface-inset);color:var(--text-secondary);cursor:pointer;text-align:left}.kf-accent-chip:hover{border-color:var(--border-strong);background:var(--surface-hover)}.kf-accent-chip[aria-pressed="true"]{border-color:var(--accent);background:var(--surface-selected);color:var(--text);box-shadow:inset 0 0 0 1px rgba(var(--accent-rgb),.12)}.kf-accent-chip strong{min-width:0;font-size:9px;line-height:1.15}.kf-swatch{width:18px;height:30px;border-radius:var(--radius-xs);border:1px solid rgba(255,255,255,.24)}.kf-swatch[data-color="kick"]{background:#7cff2b}.kf-swatch[data-color="cyan"]{background:#38d7d0}.kf-swatch[data-color="violet"]{background:#9667ff}.kf-swatch[data-color="gold"]{background:#ffbe2e}.kf-swatch[data-color="custom"]{background:var(--kf-custom-accent,#ff5ca8)}.kf-custom-color{display:flex;align-items:center;gap:10px;width:100%;padding:9px 10px;border:1px solid var(--border);border-radius:var(--radius-md);background:var(--surface-inset)}.kf-custom-color input{width:42px;height:34px;padding:2px;border:0;border-radius:var(--radius-sm);background:transparent;cursor:pointer}.kf-custom-color strong{color:var(--text);font-size:11px}.kf-custom-color small{display:block;margin-top:2px;color:var(--muted);font-size:9px}.kf-custom-accent-row[data-visible="false"]{display:none}.kf-appearance-layout{display:grid;grid-template-columns:minmax(0,1.3fr) minmax(300px,.7fr);gap:24px}.kf-appearance-controls{min-width:0}.kf-appearance-controls .kf-row,.kf-appearance-controls .kf-row-wide{min-height:0;grid-template-columns:1fr;gap:8px;padding:11px 0}.kf-appearance-controls .kf-row:has(.kf-control){min-height:64px;grid-template-columns:minmax(160px,1fr) minmax(190px,auto);align-items:center;gap:12px}.kf-appearance-controls .kf-row:has(.kf-control) p{max-width:200px}.kf-appearance-controls .kf-control{width:190px;min-width:0;justify-content:flex-end}.kf-appearance-controls .kf-segmented{width:100%}.kf-appearance-controls .kf-segmented button{min-width:0;flex:1;padding-inline:8px}.kf-appearance-controls .kf-range{grid-template-columns:38px minmax(90px,1fr) 34px;gap:6px}.kf-preview{position:sticky;top:0;align-self:start;min-width:0;padding-left:20px;border-left:1px solid var(--border)}.kf-preview-kicker{color:var(--accent);font-size:10px;font-weight:850;letter-spacing:.1em;text-transform:uppercase}.kf-preview-intro{margin:3px 0 14px;color:var(--muted);font-size:11px}.kf-preview-surface{overflow:hidden;border:1px solid var(--border);border-radius:var(--radius-md);background:var(--surface-inset);box-shadow:var(--shadow-control)}.kf-preview-surface header{display:flex;align-items:center;gap:12px;min-height:48px;padding:0 12px;border-bottom:1px solid var(--border-subtle);font-size:9px}.kf-preview-surface header strong{margin-right:auto;color:var(--accent);font-size:12px}.kf-preview-surface header span{color:var(--muted)}.kf-preview-image{display:block;width:100%;aspect-ratio:16 / 9;object-fit:cover;border-bottom:1px solid var(--border-subtle);filter:saturate(var(--kf-thumb-saturation,1.03))}.kf-preview-feature{padding:18px 14px;border-bottom:1px solid var(--border-subtle)}.kf-preview-feature h3{margin:7px 0 3px;font-size:15px;line-height:1.2}.kf-preview-feature p{margin:0;color:var(--muted);font-size:10px}.kf-preview-live{display:inline-flex;align-items:center;gap:7px;color:#dce3de;font-size:10px;font-weight:800;letter-spacing:.06em;text-transform:uppercase}.kf-preview-live::before{content:'';width:7px;height:7px;border-radius:var(--radius-xxs);background:var(--accent)}.kf-preview-action{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-top:18px;color:var(--muted);font-size:9px}.kf-preview-action b{padding:7px 10px;border:1px solid var(--accent);border-radius:var(--radius-xs);color:var(--accent);font-size:9px}.kf-preview-list{display:flex;justify-content:space-between;gap:10px;padding:12px 14px;border-bottom:1px solid var(--border-subtle);font-size:9px}.kf-preview-list:last-child{border-bottom:0}.kf-preview-list span{color:var(--muted)}.kf-preview-list strong{text-align:right}.kf-status-card{display:grid;grid-template-columns:1fr auto;align-items:center;gap:18px;padding:18px 0;border-bottom:1px solid var(--border-subtle)}.kf-status-card h3{margin:0 0 3px;font-size:15px}.kf-status-card p{max-width:560px;margin:0;color:var(--muted);font-size:12px;line-height:1.5}.kf-active{color:var(--accent);font-weight:800}.kf-stats{min-width:0;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));border-bottom:1px solid var(--border-subtle)}.kf-stat{min-width:0;padding:15px 12px;border-left:1px solid var(--border-subtle);text-align:left}.kf-stat:first-child{border-left:0}.kf-stat span{display:block;color:var(--muted);font-size:9px;letter-spacing:.06em;text-transform:uppercase}.kf-stat strong{display:block;overflow:hidden;margin-top:4px;color:var(--accent);font-size:15px;text-overflow:ellipsis;white-space:nowrap}.kf-defense-overview{min-width:0;display:grid;grid-template-columns:minmax(0,1.35fr) minmax(280px,.65fr);border-bottom:1px solid var(--border)}.kf-defense-overview .kf-status-card,.kf-defense-overview .kf-stats{border-bottom:0}.kf-defense-overview .kf-status-card{padding-right:18px}.kf-defense-overview .kf-status-card>.kf-active{display:none}.kf-defense-overview .kf-stats{border-left:1px solid var(--border-subtle)}.kf-content-section{margin-top:18px}.kf-content-section{overflow:hidden;padding:0 16px;border:1px solid var(--border);border-radius:var(--radius-md);background:var(--surface-inset);box-shadow:0 1px 0 rgba(255,255,255,.015) inset}.kf-content-section .kf-subsection-header{margin:0 -16px;padding:14px 16px 12px;border-bottom-color:var(--border);background:var(--surface-2)}[data-kf-current-page="content"] .kf-row{min-height:62px;padding:10px 0}[data-kf-current-page="content"] .kf-row h3{margin-bottom:2px;font-size:12px}[data-kf-current-page="content"] .kf-row p{font-size:11px}[data-kf-current-page="content"] .kf-subsection{margin-top:20px}[data-kf-current-page="content"] .kf-status-note{font-size:10px}.kf-tool-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin-top:20px}.kf-tool-card{min-height:96px;display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:center;gap:14px;padding:14px;border:1px solid var(--border);border-radius:var(--radius-md);background:var(--surface-inset)}.kf-tool-card:hover{border-color:var(--border-control);background:var(--surface-hover)}.kf-tool-card h3{margin:0 0 3px;font-size:11px}.kf-tool-card p{margin:0;color:var(--muted);font-size:11px;line-height:1.45}.kf-channel-input-row{display:grid;grid-template-columns:minmax(220px,1fr) auto;gap:9px;align-items:center}.kf-channel-list{display:grid;gap:6px;margin-top:10px;max-height:280px;overflow:auto;scrollbar-gutter:stable}.kf-channel-entry{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:6px 10px;border:1px solid var(--border-subtle);border-radius:var(--radius-sm);background:var(--surface-inset);font-size:13px}.kf-channel-entry span{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;min-width:0}.kf-emote-manager{margin-top:18px}.kf-sticker-library-workspace{min-width:0;display:grid;grid-template-columns:210px minmax(0,1fr);gap:12px}.kf-sticker-group-panel{min-width:0;padding:12px;border:1px solid var(--border);border-radius:var(--radius-md);background:var(--surface-inset)}.kf-sticker-group-heading{display:flex;align-items:center;justify-content:space-between;gap:8px}.kf-sticker-group-heading h4{margin:0;font-size:13px}.kf-sticker-group-heading span{color:var(--muted);font-size:11px}.kf-sticker-group-panel>p{margin:4px 0 10px;color:var(--muted);font-size:11px;line-height:1.45}.kf-sticker-library-main{min-width:0}.kf-sticker-library-controls{display:grid;grid-template-columns:minmax(220px,1fr) 180px;gap:9px}.kf-sticker-library-controls .kf-select{width:100%;height:40px}.kf-sticker-group-builder{display:grid;gap:7px}.kf-sticker-group-list{display:grid;gap:7px;margin-top:10px}.kf-sticker-group-row{display:grid;gap:5px}.kf-sticker-group-row .kf-text{min-height:34px;padding-block:6px}.kf-sticker-group-row-actions{display:flex;min-width:0;align-items:center;gap:4px}.kf-sticker-group-row-actions>span{min-width:18px;margin-right:auto;color:var(--muted);font-size:10px}.kf-sticker-group-empty{color:var(--muted);font-size:10px}.kf-sticker-library-bulk{display:grid;grid-template-columns:auto auto auto minmax(120px,1fr) auto auto;gap:6px;align-items:center;margin-top:9px;padding:8px;border:1px solid var(--border);border-radius:var(--radius-sm);background:var(--surface-2)}.kf-sticker-library-bulk>strong{color:var(--text-secondary);font-size:11px;white-space:nowrap}.kf-sticker-library-bulk .kf-select{min-width:0;width:100%}.kf-sticker-library-meta{display:flex;align-items:center;justify-content:space-between;gap:12px;margin:13px 0 8px;color:var(--muted);font-size:10px}.kf-sticker-library-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(270px,1fr));gap:8px;max-height:520px;overflow:auto;padding-right:4px;scrollbar-gutter:stable}.kf-my-emote-group{margin-top:14px;padding-top:12px;border-top:1px solid var(--border)}.kf-my-emote-group:first-of-type{margin-top:8px}.kf-my-emote-group>header{display:flex;align-items:end;justify-content:space-between;gap:16px;margin-bottom:8px}.kf-my-emote-group>header span{color:var(--accent);font-size:8px;font-weight:850;letter-spacing:.09em;text-transform:uppercase}.kf-my-emote-group>header h4{margin:3px 0 0;color:var(--text);font-size:13px}.kf-my-emote-group>header>strong{color:var(--muted);font-size:10px}.kf-my-emote-group .kf-sticker-library-grid{max-height:none;overflow:visible;padding-right:0}.kf-sticker-library-item{min-width:0;display:grid;grid-template-columns:52px minmax(0,1fr);gap:10px;padding:9px;border:1px solid var(--border-subtle);border-radius:var(--radius-sm);background:var(--surface-inset);content-visibility:auto;contain-intrinsic-size:auto 86px}.kf-sticker-library-item[data-removed="true"]{opacity:.58}.kf-sticker-library-item[data-selected="true"]{border-color:var(--accent);box-shadow:0 0 0 1px rgba(var(--accent-rgb),.18)}.kf-sticker-library-image{width:52px;height:52px;display:grid;place-items:center;padding:5px;border:1px solid var(--border);border-radius:var(--radius-sm);background:var(--surface-2)}.kf-sticker-library-image img{width:100%;height:100%;object-fit:contain}.kf-sticker-library-copy{min-width:0}.kf-sticker-library-copy strong{display:block;overflow:hidden;font-size:13px;text-overflow:ellipsis;white-space:nowrap}.kf-sticker-library-copy small{display:block;overflow:hidden;margin-top:2px;color:var(--muted);font-size:11px;text-overflow:ellipsis;white-space:nowrap}.kf-sticker-access{display:inline-flex;margin-top:5px;padding:2px 5px;border:1px solid var(--border-strong);border-radius:var(--radius-sm);color:var(--muted);font-size:9px;font-weight:800;letter-spacing:.04em;text-transform:uppercase}.kf-sticker-access[data-access="available"]{border-color:rgba(139,232,92,.55);color:#8be85c}.kf-sticker-access[data-access="channel"]{border-color:rgba(255,190,46,.58);color:#ffcf61}.kf-sticker-access[data-access="observed"]{border-color:rgba(56,215,208,.58);color:#70e9e3}.kf-emote-catalog-browser{display:grid;grid-template-columns:minmax(170px,.7fr) minmax(260px,1.3fr);gap:12px;align-items:center;margin-bottom:12px;padding:12px;border:1px solid var(--border);border-radius:var(--radius-md);background:var(--surface-2)}.kf-emote-catalog-browser h4{margin:0;color:var(--text);font-size:13px}.kf-emote-catalog-browser p{margin:2px 0 0;color:var(--muted);font-size:11px;line-height:1.45}.kf-emote-catalog-status{grid-column:1 / -1}.kf-emote-catalog-form{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px}.kf-emote-catalog-status[data-error="true"]{color:var(--danger-text)}.kf-sticker-changed{display:inline-flex;margin:5px 0 0 5px;padding:2px 5px;border:1px solid rgba(217,139,58,.62);border-radius:var(--radius-xs);color:#e0a367;font-size:8px;font-weight:800;letter-spacing:.04em;text-transform:uppercase}.kf-sticker-library-item[data-changed="true"]{border-color:rgba(217,139,58,.42)}.kf-sticker-lock{display:block;margin-top:5px;color:var(--muted);font-size:9px;line-height:1.5;white-space:normal}.kf-sticker-lock a{color:var(--accent)}.kf-sticker-library-actions{grid-column:1 / -1;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:6px}.kf-sticker-library-actions .kf-select{grid-column:1 / -1;min-width:0;width:100%}.kf-table{width:100%;border-collapse:collapse;font-size:12px}.kf-table th,.kf-table td{padding:11px 9px;border-bottom:1px solid var(--border-subtle);text-align:left;vertical-align:middle}.kf-table th{color:var(--text-secondary);background:transparent;font-size:10px;letter-spacing:.07em;text-transform:uppercase}.kf-table .kf-muted{color:var(--muted)}.kf-table tr:last-child td{border-bottom:0}.kf-table .kf-table-actions{text-align:right}.kf-shortcut{display:inline-flex;min-width:62px;justify-content:center;padding:4px 8px;border:1px solid var(--border-control);border-radius:var(--radius-sm);background:var(--surface-2);font-weight:700}.kf-conflict td{background:rgba(255,98,88,.055);border-top:1px solid var(--danger);border-bottom:1px solid var(--danger)}.kf-conflict-message{color:var(--danger);font-size:11px}.kf-status-note{margin-top:12px;padding:10px 12px;border-left:2px solid var(--border-strong);background:rgba(255,255,255,.018);color:var(--muted);font-size:11px}.kf-status-note[data-drifted="true"]{border-color:#7b5d20;background:rgba(246,185,67,.065);color:#e7c77e}.kf-notice{margin-top:12px;padding:11px 13px;border-left:2px solid #997326;background:rgba(246,185,67,.055);color:#e7c77e;font-size:11px}.kf-subsection{margin-top:26px}.kf-subsection-header{display:flex;align-items:end;justify-content:space-between;gap:16px;margin-bottom:10px;padding-bottom:11px;border-bottom:1px solid var(--border)}.kf-subsection h3{margin:0;font-size:13px;letter-spacing:.02em}.kf-subsection p{margin:2px 0 0;color:var(--muted);font-size:11px}.kf-about-status{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;padding:18px 0;border-bottom:1px solid var(--border-subtle)}.kf-mini-card{padding:14px;border:1px solid var(--border);border-radius:var(--radius-md);background:var(--surface-inset)}.kf-mini-card span{display:block;color:var(--muted);font-size:9px;letter-spacing:.07em;text-transform:uppercase}.kf-mini-card strong{display:block;margin-top:4px;color:var(--accent)}.kf-about-status .kf-mini-card[data-status="warning"]{border-color:color-mix(in srgb,var(--warning) 52%,var(--border))}.kf-about-status .kf-mini-card[data-status="warning"] strong{color:var(--warning)}.kf-about-status .kf-mini-card[data-status="neutral"] strong,.kf-about-status .kf-mini-card[data-status="checking"] strong{color:var(--text-secondary)}.kf-nav-earned:empty{display:none}.kf-nav-earned{display:block;margin-top:3px;color:var(--accent);font-size:11px}.kf-nav-earned::before{content:'● '}[data-kf-earned="reward-ready"]{position:relative}[data-kf-earned="reward-ready"]::after{content:'';position:absolute;top:4px;right:4px;width:7px;height:7px;border:1px solid var(--surface-2);border-radius:var(--radius-xs);background:var(--accent);animation:kf-earned-pulse 2.4s ease-in-out infinite}@keyframes kf-earned-pulse{0%,100%{opacity:1}50%{opacity:.45}}@media (prefers-reduced-motion:reduce){[data-kf-earned="reward-ready"]::after{animation:none}}.kf-layout-save{display:grid;gap:8px;justify-items:stretch;min-width:240px}.kf-chip-row{display:flex;flex-wrap:wrap;gap:6px}.kf-chip{padding:5px 10px;color:var(--muted);border:1px solid var(--border);border-radius:var(--radius-sm);background:transparent;font-size:11px;cursor:pointer}.kf-chip[aria-pressed="true"]{color:var(--accent);border-color:var(--accent)}.kf-layout-list{display:grid;gap:6px;padding:10px 0 2px}.kf-layout-entry{display:grid;grid-template-columns:1fr auto;align-items:center;gap:12px;padding:10px 12px;border:1px solid var(--border);border-radius:var(--radius-md);background:var(--surface-inset)}.kf-layout-entry[data-active="true"]{border-color:var(--accent)}.kf-layout-entry span{display:block;margin-top:2px;color:var(--muted);font-size:11px}.kf-chat-log{display:grid;gap:8px;min-width:0}.kf-chat-log-row{display:grid;grid-template-columns:auto auto 1fr;gap:8px;padding:4px 0;border-bottom:1px solid var(--border-subtle);font-size:12px}.kf-chat-log-row span:first-child{color:var(--muted);font-variant-numeric:tabular-nums}.kf-chat-log-row strong{color:var(--accent)}.kf-chat-log-row span:last-child{overflow-wrap:anywhere}.kf-hub-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;padding:18px 0}.kf-hub-card{position:relative;overflow:hidden;min-height:118px;padding:15px;border:1px solid var(--border);border-radius:var(--radius-md);background:linear-gradient(145deg,var(--surface-inset),var(--surface-1))}.kf-hub-card::before{content:'';position:absolute;inset:0 0 auto;height:2px;background:var(--accent);opacity:.32}.kf-hub-card>span{display:block;color:var(--subtle);font-size:9px;font-weight:800;letter-spacing:.08em;text-transform:uppercase}.kf-hub-card>strong{display:block;margin-top:7px;color:var(--accent);font-size:19px}.kf-hub-card em{display:block;margin-top:6px;color:var(--muted);font-size:11px;font-style:normal;line-height:1.45}.kf-hub-card[data-state="unavailable"] strong,.kf-hub-card[data-state="loading"] strong{color:var(--muted)}.kf-hub-card[data-state="unavailable"]::before,.kf-hub-card[data-state="loading"]::before{background:var(--border-control);opacity:.45}.kf-hub-card[data-state="error"] strong{color:var(--danger-text)}.kf-hub-card[data-state="error"]::before{background:var(--danger);opacity:.8}.kf-action-row{min-height:78px;display:grid;grid-template-columns:1fr auto;align-items:center;gap:22px;padding:14px 0;border-bottom:1px solid var(--border-subtle)}.kf-action-row h3{margin:0 0 4px;font-size:13px}.kf-action-row p{max-width:560px;margin:0;color:var(--muted);font-size:12px;line-height:1.5}.kf-action-warning{margin-inline:-16px;padding-inline:14px;border-left:2px solid var(--warning);background:color-mix(in srgb,var(--warning) 5%,transparent)}.kf-danger{border-color:rgba(255,98,88,.65)!important;color:var(--danger-text)!important}[data-kf-current-page="accessibility"]{padding-top:14px}[data-kf-current-page="accessibility"] .kf-page-header{min-height:70px;padding-bottom:12px}[data-kf-current-page="accessibility"] .kf-row{min-height:60px;padding:9px 0}[data-kf-current-page="accessibility"] .kf-subsection{margin-top:14px}[data-kf-current-page="accessibility"] .kf-subsection-header{margin-bottom:0;padding-bottom:8px}[data-kf-current-page="accessibility"] .kf-table th,[data-kf-current-page="accessibility"] .kf-table td{padding-block:7px}[data-kf-current-page="accessibility"] .kf-button-small{min-height:32px}[data-kf-current-page="layout"]>.kf-panel,[data-kf-current-page="accessibility"]>.kf-panel,[data-kf-current-page="about"]>.kf-panel{margin-top:16px;padding:0 16px;border:1px solid var(--border);border-radius:var(--radius-md);background:var(--surface-inset)}[data-kf-current-page="about"] .kf-action-row{min-height:78px;padding-block:13px}[data-kf-current-page="about"] .kf-subsection{margin-top:18px}[data-kf-current-page="about"] .kf-subsection>.kf-panel{overflow:hidden;border:1px solid var(--border);border-radius:var(--radius-sm)}.kf-button{min-height:var(--control-height);display:inline-flex;align-items:center;justify-content:center;gap:8px;padding:0 14px;border:1px solid var(--border-control);border-radius:var(--radius-md);background:var(--surface-3);color:var(--text);cursor:pointer;font-size:12px;font-weight:700}.kf-button:hover{border-color:var(--border-strong);background:var(--surface-hover);transform:translateY(-1px);box-shadow:var(--shadow-control)}.kf-button:active{transform:translateY(0);box-shadow:none}.kf-button-primary{border-color:var(--accent);background:var(--accent);color:var(--on-accent)}.kf-button-primary:hover{border-color:var(--accent);background:var(--accent);filter:brightness(1.08)}.kf-button:disabled{opacity:.48;cursor:not-allowed;transform:none;box-shadow:none}.kf-button-primary:disabled{opacity:1;border-color:var(--border);background:var(--surface-hover);color:var(--muted)}.kf-button-small{min-height:var(--control-height-small);padding-inline:10px;font-size:11px}.kf-button .kf-icon{width:16px;height:16px}.kf-button-group{display:flex;gap:8px;justify-content:flex-end;flex-wrap:wrap}.kf-footer{min-width:0;display:flex;align-items:center;justify-content:space-between;gap:16px;padding:0 22px;border-top:1px solid var(--border);background:var(--surface-2)}.kf-footer-left,.kf-footer-right{min-width:0;display:flex;align-items:center;gap:10px}.kf-toast{position:fixed;right:24px;bottom:24px;z-index:2147483500;max-width:430px;padding:11px 14px;border:1px solid var(--border);border-radius:var(--radius-sm);background:var(--surface-3);box-shadow:0 18px 48px rgba(0,0,0,.5);color:var(--text)}.kf-toast[data-error="true"]{border-color:var(--danger);background:var(--surface-danger)}.kf-toast:has(.kf-toast-action){display:flex;align-items:center;gap:10px}[data-kf-settings-backdrop]:not([hidden])~.kf-toast:not([hidden]){bottom:120px}[data-kf-multistream-backdrop]:not([hidden])~.kf-toast:not([hidden]){bottom:150px}.kf-toast-text{flex:1 1 auto}.kf-toast-action{flex:0 0 auto;padding:5px 10px;border:1px solid var(--accent);border-radius:var(--radius-sm);background:transparent;color:var(--accent);font:inherit;font-weight:650;cursor:pointer}.kf-toast-action:hover{background:var(--accent);color:var(--on-accent)}.kf-toast-action:focus-visible{outline:var(--focus-ring);outline-offset:var(--focus-offset)}.kf-storage-alert{position:fixed;z-index:2147483001;inset-inline:16px;bottom:16px;margin-inline:auto;max-width:640px;display:flex;align-items:center;gap:12px;padding:12px 14px;border:1px solid var(--danger);border-radius:var(--radius-lg);background:var(--surface-danger);color:var(--text);box-shadow:0 18px 44px rgba(0,0,0,.55);font-size:12px}.kf-storage-alert[hidden]{display:none}.kf-storage-alert-body{display:grid;gap:2px;flex:1;min-width:0}.kf-storage-alert-body strong{font-size:12px}.kf-storage-alert-body span{opacity:.85}.kf-command-shell{width:min(560px,calc(100vw - 48px));max-height:min(620px,calc(100vh - 80px));overflow:hidden;border:1px solid var(--border);border-radius:var(--radius-lg);background:var(--surface-1);box-shadow:var(--shadow-dialog);color:var(--text)}.kf-command-head{display:grid;grid-template-columns:1fr auto;align-items:end;gap:7px 12px;padding:16px;border-bottom:1px solid var(--border)}.kf-command-head label{color:var(--text-secondary);font-size:11px;font-weight:780;letter-spacing:.05em;text-transform:uppercase}.kf-command-head span{color:var(--muted);font-size:10px}.kf-command-head input{grid-column:1 / -1;width:100%;height:var(--control-height);padding:0 13px;border:1px solid var(--border-control);border-radius:var(--radius-md);background:var(--surface-inset);color:var(--text)}.kf-command-head input:focus-visible{border-color:var(--accent)}@media (forced-colors:active){.kf-text:focus-visible,.kf-textarea:focus-visible,.kf-command-head input:focus-visible,.kf-ms-head input:focus-visible,.kf-ms-foot input:focus-visible,.kf-select:focus-visible,input:focus-visible,select:focus-visible,textarea:focus-visible,button:focus-visible,[tabindex]:focus-visible{outline:3px solid Highlight;outline-offset:2px}.kf-panel,.kf-settings,.kf-command-shell{border:1px solid CanvasText}.kf-storage-alert{border:2px solid CanvasText}.kf-switch{border:1px solid CanvasText}.kf-switch[aria-checked="true"]{background:Highlight}[aria-checked="true"],[aria-selected="true"],[aria-pressed="true"],[aria-current="page"]{outline:2px solid Highlight;outline-offset:1px}.kf-sticker-library-item[data-selected="true"],.kf-ms-tile[data-kf-multistream-focused="true"]{outline:2px solid Highlight;outline-offset:1px}.kf-toast{border:1px solid CanvasText}button:disabled,[aria-disabled="true"],.kf-button:disabled{border-color:GrayText;color:GrayText}}:host :is(button,a[href],input,select,textarea,[role="button"],[role="switch"],[role="option"]){min-width:24px;min-height:24px}:host p a[href],:host small a[href],:host li a[href]{min-width:0;min-height:0}:host [data-kf-page] :is(button,a[href],input,select,textarea,[role="switch"]){scroll-margin-block:72px}:host([data-kf-large-targets="true"]) :is(button,a[href],input,select,textarea){min-height:40px}:host([data-kf-large-targets="true"]) .kf-switch{min-width:74px}:host([data-kf-large-targets="true"]) .kf-icon-button{min-width:40px}:host([data-kf-large-targets="true"]) .kf-ms-bar :is(button,.kf-ms-link){min-height:32px;padding:6px 10px}:host([data-kf-large-targets="true"]) .kf-ms-bar{opacity:1}:host([data-kf-reduce-motion="true"]) *,:host([data-kf-reduce-motion="true"]) *::before,:host([data-kf-reduce-motion="true"]) *::after{animation-duration:.001ms!important;animation-iteration-count:1!important;transition-duration:.001ms!important;scroll-behavior:auto!important}.kf-fact-list{margin:0;padding:0;display:grid;gap:10px}.kf-fact{margin:0;padding:10px 12px;border-left:3px solid var(--border-subtle);background:rgba(255,255,255,.02);border-radius:var(--radius-none) var(--radius-sm) var(--radius-sm) var(--radius-none)}.kf-fact dt{margin:0 0 3px;font-size:12px;font-weight:700}.kf-fact dd{margin:0;color:var(--muted);font-size:11px;line-height:1.55}.kf-ms-backdrop{padding:0}.kf-ms-shell{display:grid;grid-template-rows:auto auto 1fr auto;width:100vw;height:100vh;background:radial-gradient(circle at 50% -12%,rgba(var(--accent-rgb),.06),transparent 34%),var(--surface-0);color:var(--text)}.kf-ms-head,.kf-ms-foot{display:grid;align-items:center;gap:12px;padding:12px 16px;border-bottom:1px solid var(--border);background:var(--surface-2)}.kf-ms-head{grid-row:1}.kf-ms-error{grid-row:2}.kf-ms-body{grid-row:3}.kf-ms-foot{grid-row:4}.kf-ms-head{grid-template-columns:minmax(150px,auto) minmax(300px,1fr) auto}.kf-ms-foot{grid-template-columns:auto minmax(0,1fr);border-bottom:0;border-top:1px solid var(--border)}.kf-ms-brand{display:grid;gap:2px}.kf-ms-brand strong{font-size:15px;letter-spacing:-.015em}.kf-ms-add{display:flex;align-items:center;justify-content:center;gap:7px;min-width:0}.kf-ms-add input{width:min(420px,100%)}.kf-ms-controls{display:flex;align-items:center;justify-content:flex-end;gap:6px;flex-wrap:wrap}.kf-ms-backdrop[data-kf-multistream-empty="true"] .kf-ms-controls>:not([data-action="close-multistream"]){display:none}.kf-ms-save-layout{display:flex;align-items:center;gap:7px}.kf-ms-points-note{grid-column:1 / -1;display:flex;align-items:center;gap:7px;margin:0;color:var(--muted);font-size:10px;line-height:1.45}.kf-ms-points-note svg{width:14px;height:14px;flex:0 0 14px;color:var(--warning)}.kf-ms-count{color:var(--muted);font-size:11px}.kf-ms-head input,.kf-ms-foot input{min-width:220px;min-height:32px;padding:0 10px;border:1px solid var(--border-control);border-radius:var(--radius-md);background:var(--surface-inset);color:var(--text);font-size:12px}.kf-ms-head input:focus-visible,.kf-ms-foot input:focus-visible{border-color:var(--accent)}.kf-ms-select{min-height:32px;font-size:12px}.kf-ms-error{padding:9px 14px;border-bottom:1px solid var(--danger);background:var(--surface-danger);color:var(--danger-text);font-size:12px}.kf-ms-error[hidden]{display:none}.kf-ms-body{display:grid;grid-template-columns:1fr 0;min-height:0}.kf-ms-backdrop[data-kf-multistream-show-chat="true"] .kf-ms-body{grid-template-columns:1fr 340px}.kf-ms-grid{display:grid;grid-template-columns:repeat(var(--kf-multistream-columns,1),1fr);gap:8px;padding:10px;min-height:0;align-content:stretch}.kf-ms-empty-state{grid-column:1 / -1;align-self:center;justify-self:center;width:min(520px,calc(100% - 36px));display:grid;justify-items:center;gap:10px;padding:34px;border:1px solid var(--border);border-radius:var(--radius-lg);background:linear-gradient(145deg,var(--surface-1),var(--surface-inset));box-shadow:var(--shadow-control);text-align:center}.kf-ms-empty-state img{width:48px;height:48px;object-fit:contain}.kf-ms-empty-state span{color:var(--accent);font-size:9px;font-weight:850;letter-spacing:.12em;text-transform:uppercase}.kf-ms-empty-state h2{margin:0;font-size:23px;letter-spacing:-.035em}.kf-ms-empty-state p{max-width:420px;margin:0 0 4px;color:var(--muted);font-size:12px;line-height:1.55}.kf-ms-tile{position:relative;min-height:0;border:1px solid var(--border);border-radius:var(--radius-md);overflow:hidden;background:#000}.kf-ms-tile[data-kf-multistream-focused="true"]{border-color:var(--accent);box-shadow:0 0 0 1px var(--accent) inset}.kf-ms-backdrop[data-kf-multistream-paused="true"] .kf-ms-tile::after{content:attr(data-kf-multistream-tile) " (paused)";position:absolute;inset:0;display:grid;place-items:center;background:var(--surface-inset);color:var(--muted);font-size:12px;font-weight:700}.kf-ms-backdrop[data-kf-multistream-paused="true"] .kf-ms-bar{z-index:1}.kf-ms-tile iframe{width:100%;height:100%;border:0;display:block}.kf-ms-bar{position:absolute;inset-inline:0;top:0;display:flex;align-items:center;gap:6px;padding:4px 6px;background:linear-gradient(180deg,rgba(0,0,0,.78),transparent);opacity:.88;transition:opacity .12s ease;font-size:11px}.kf-ms-bar .kf-ms-spacer{flex:1}.kf-ms-tile:hover .kf-ms-bar,.kf-ms-tile:focus-within .kf-ms-bar{opacity:1}.kf-ms-bar button,.kf-ms-bar .kf-ms-link{border:1px solid rgba(255,255,255,.25);border-radius:var(--radius-sm);background:rgba(0,0,0,.55);color:var(--text);padding:2px 7px;font-size:11px;font-weight:700;cursor:pointer;text-decoration:none}.kf-ms-bar button:hover,.kf-ms-bar .kf-ms-link:hover{border-color:var(--accent);color:var(--accent)}.kf-ms-tile[data-kf-multistream-focused="true"] .kf-ms-name{border-color:var(--accent);color:var(--accent)}.kf-ms-merged{min-width:0;border-left:1px solid var(--border);display:grid;grid-template-rows:auto auto 1fr}.kf-ms-chat-status{margin:0;padding:4px 10px;border-bottom:1px solid var(--border);color:var(--text-secondary);font-size:11px;font-variant-numeric:tabular-nums}.kf-ms-merged-list{margin:0;padding:6px 8px;overflow-y:auto;list-style:none;font-size:12px;line-height:1.45}.kf-ms-merged-row{padding:2px 0;overflow-wrap:anywhere}.kf-ms-merged-source{display:inline-block;margin-right:6px;padding:0 5px;border-radius:var(--radius-sm);background:var(--kf-panel-high,#18201b);color:var(--kf-accent,#7cff2b);font-size:11px;font-weight:700}.kf-ms-merged-who{margin-right:4px;font-weight:700}.kf-ms-merged-who::after{content:':'}.kf-ms-backdrop[data-kf-multistream-merged-on="true"] .kf-ms-chat{display:none}.kf-ms-chat{min-width:0;border-left:1px solid var(--border);display:grid;grid-template-rows:auto 1fr}.kf-ms-backdrop[data-kf-multistream-chat-popped-out="true"] .kf-ms-chat{display:none}.kf-ms-chat iframe{width:100%;height:100%;border:0;display:block}.kf-ms-chat-notice{margin:0;padding:6px 10px;border-bottom:1px solid var(--border);background:var(--surface-2);color:var(--muted);font-size:11px}.kf-ms-layouts{display:flex;align-items:center;gap:6px;flex-wrap:wrap}.kf-ms-layout{display:inline-flex}.kf-ms-layout button{border:1px solid var(--border);background:var(--surface-inset);color:var(--text);padding:3px 8px;font-size:11px;cursor:pointer}.kf-ms-layout button:first-child{border-radius:var(--radius-md) var(--radius-none) var(--radius-none) var(--radius-md)}.kf-ms-layout button:last-child{border-radius:var(--radius-none) var(--radius-md) var(--radius-md) var(--radius-none);border-left:0}.kf-ms-layout button:hover{border-color:var(--accent);color:var(--accent)}.kf-ms-layout small{opacity:.6}.kf-ms-layout small.kf-ms-live{opacity:1;color:var(--muted)}.kf-ms-layout small.kf-ms-live[data-live="true"]{color:var(--accent);font-weight:700}.kf-ms-empty{color:var(--muted);font-size:11px}@media (max-width:1360px){.kf-ms-head{grid-template-columns:auto minmax(280px,1fr)}.kf-ms-controls{grid-column:1 / -1;justify-content:flex-end}}@media (max-width:760px){.kf-ms-head,.kf-ms-foot{grid-template-columns:1fr}.kf-ms-add,.kf-ms-save-layout{justify-content:stretch}.kf-ms-add input,.kf-ms-save-layout input{width:100%;min-width:0}.kf-ms-layouts,.kf-ms-points-note{grid-column:1}.kf-ms-empty-state{padding:24px 18px}}.kf-shadow-warning{display:grid;gap:6px}.kf-shadow-warning code{font-size:11px;color:var(--accent)}.kf-command-list{max-height:490px;overflow:auto;padding:8px}.kf-command-item{width:100%;display:grid;grid-template-columns:1fr auto;align-items:center;gap:14px;padding:12px;border:1px solid transparent;border-left:3px solid transparent;border-radius:var(--radius-sm);background:transparent;text-align:left;cursor:pointer}.kf-command-item:hover,.kf-command-item[data-active="true"]{border-color:var(--border-subtle);border-left-color:var(--accent);background:var(--surface-hover)}.kf-command-item strong{display:block;margin-bottom:2px}.kf-command-item span{color:var(--muted);font-size:12px}.kf-command-empty{display:grid;gap:4px;padding:38px 28px;color:var(--muted);text-align:center}.kf-command-empty strong{color:var(--text);font-size:13px}.kf-command-empty span{font-size:11px}.kf-sr-only{position:absolute!important;width:1px!important;height:1px!important;padding:0!important;margin:-1px!important;overflow:hidden!important;clip:rect(0,0,0,0)!important;white-space:nowrap!important;border:0!important}:is(button,input,select,textarea):focus-visible{outline:var(--focus-ring);outline-offset:var(--focus-offset)}@media (max-width:920px){.kf-settings{width:calc((100vw - 28px) / var(--kf-interface-scale,1));height:calc((100vh - 28px) / var(--kf-interface-scale,1));min-width:0;min-height:calc(620px / var(--kf-interface-scale,1))}.kf-header{grid-template-columns:1fr auto auto;gap:14px;padding-inline:18px}.kf-body{grid-template-columns:204px minmax(0,1fr)}.kf-nav button{grid-template-columns:21px minmax(0,1fr);gap:10px;padding-inline:18px}.kf-nav .kf-nav-copy{overflow:visible;white-space:normal}.kf-nav-copy>strong{line-height:1.15;white-space:normal}.kf-nav-copy>span{display:none}.kf-page{padding-inline:24px 20px}.kf-row,.kf-row-wide{grid-template-columns:1fr;gap:10px}.kf-control{min-width:0;justify-content:flex-start}.kf-range{max-width:420px}.kf-defense-overview{grid-template-columns:1fr}.kf-defense-overview .kf-stats{border-left:0;border-top:1px solid var(--border-subtle)}.kf-tool-grid{grid-template-columns:1fr}.kf-sticker-library-workspace,.kf-sticker-library-controls,.kf-sticker-library-grid{grid-template-columns:1fr}.kf-sticker-group-builder{grid-template-columns:minmax(180px,1fr) auto}.kf-sticker-library-bulk{grid-template-columns:repeat(3,minmax(0,1fr))}.kf-sticker-library-bulk>strong,.kf-sticker-library-bulk .kf-select{grid-column:span 2}.kf-sticker-library-actions{grid-template-columns:repeat(2,auto)}.kf-sticker-library-actions .kf-select{grid-column:1 / -1}}@media (max-width:700px){.kf-backdrop{padding:0}.kf-settings{width:calc(100vw / var(--kf-interface-scale,1));height:calc(100vh / var(--kf-interface-scale,1));min-height:0;grid-template-rows:66px minmax(0,1fr) 68px;border:0;border-radius:var(--radius-none)}.kf-header{grid-template-columns:minmax(0,1fr) auto auto;padding-inline:14px}.kf-body{grid-template-columns:1fr;grid-template-rows:auto minmax(0,1fr)}.kf-nav{display:flex;overflow-x:auto;padding:0 0 4px;border-right:0;border-bottom:1px solid var(--border);scrollbar-color:var(--border-control) var(--surface-0);scrollbar-width:thin;scroll-snap-type:x proximity;overscroll-behavior-inline:contain}.kf-nav::-webkit-scrollbar{height:4px}.kf-nav::-webkit-scrollbar-track{background:var(--surface-0)}.kf-nav::-webkit-scrollbar-thumb{border-radius:var(--radius-xs);background:var(--border-control)}.kf-nav-search{display:none}.kf-nav button{flex:0 0 auto;width:auto;min-width:0;min-height:54px;padding-inline:16px;scroll-snap-align:start}.kf-nav-copy,.kf-nav-copy>strong{white-space:nowrap}.kf-nav button::before{inset:auto 14px 0;width:auto;height:3px}.kf-page{padding:18px 18px 32px}.kf-page-header{min-height:72px}.kf-page-header h2{font-size:23px}.kf-page-meta{display:none}.kf-control{width:100%}.kf-segmented{width:100%}.kf-segmented button{min-width:0;flex:1 1 0;padding-inline:7px}.kf-range{grid-template-columns:42px minmax(120px,1fr) 42px}.kf-channel-input-row,.kf-emote-catalog-browser,.kf-emote-catalog-form{grid-template-columns:1fr}.kf-sticker-library-bulk{grid-template-columns:repeat(2,minmax(0,1fr))}.kf-sticker-library-bulk>strong,.kf-sticker-library-bulk .kf-select{grid-column:1 / -1}.kf-theme-grid,.kf-swatch-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.kf-preset-grid{grid-template-columns:1fr}.kf-appearance-layout{grid-template-columns:1fr}.kf-preview{position:static;padding:18px 0 0;border-top:1px solid var(--border);border-left:0}.kf-about-status,.kf-stats,.kf-hub-grid{grid-template-columns:1fr}.kf-mini-card,.kf-stat{border-left:0;border-top:1px solid var(--border-subtle)}.kf-action-row{grid-template-columns:1fr}.kf-button-group{justify-content:flex-start}.kf-footer{padding-inline:14px}.kf-footer [data-action="export"]{display:none}.kf-storage-alert{align-items:stretch;flex-wrap:wrap}.kf-storage-alert-body{flex-basis:100%}}@media (max-width:430px){.kf-brand{font-size:14px}.kf-badge{display:none}.kf-save{font-size:11px}.kf-page-header p{font-size:12px}.kf-row{gap:12px;padding-block:13px}.kf-row p{font-size:11px}.kf-appearance-controls .kf-row:has(.kf-control){grid-template-columns:minmax(0,1fr)}.kf-appearance-controls .kf-control{width:100%}[data-kf-current-page="content"] .kf-table{min-width:0;table-layout:fixed}[data-kf-current-page="content"] .kf-table th,[data-kf-current-page="content"] .kf-table td{min-width:0;overflow-wrap:anywhere}[data-kf-current-page="content"] .kf-table :is(th,td):nth-child(1){width:62px}[data-kf-current-page="content"] .kf-table :is(th,td):nth-child(2){width:52px}[data-kf-current-page="content"] .kf-table :is(th,td):nth-child(4){width:64px}.kf-footer-left .kf-button{padding-inline:10px}.kf-footer [data-action="reset-page"] .kf-button-label{display:none}.kf-command-shell{width:calc(100vw - 24px)}}@media (prefers-reduced-motion:reduce){*,*::before,*::after{animation-duration:.001ms!important;transition-duration:.001ms!important}}`;
const LOCALES = ['es', 'pt'];
const TRANSLATIONS = {
'Home': ['Inicio', 'Início'],
...
const EMOTE_COMPLETION_CSS = `:host{position:fixed;z-index:2147483000;display:none;width:240px}:host([data-kf-open="true"]){display:block}:host([data-kf-anchored="true"]){inset:auto;margin:0;border:0;padding:0;background:transparent;overflow:visible;color:inherit;position-area:block-start span-inline-end;position-try-fallbacks:flip-block,flip-inline,flip-block flip-inline}[data-kf-complete-list]{display:flex;flex-direction:column;gap:2px;padding:4px;border:1px solid var(--kf-border-strong,#38463d);border-radius:12px;background:var(--kf-panel,#0b100d);box-shadow:0 12px 30px rgba(0,0,0,0.55);font:13px/1.3 system-ui,-apple-system,"Segoe UI",sans-serif;color:var(--kf-text,#f5f8f6);max-height:260px;overflow-y:auto}button{display:grid;grid-template-columns:28px 1fr;align-items:center;gap:8px;min-height:28px;padding:3px 6px;border:0;border-radius:6px;background:transparent;color:inherit;font:inherit;text-align:left;cursor:pointer}button:hover,button:focus-visible{background:rgba(var(--kf-accent-rgb,124,255,43),.12)}button:focus-visible{outline:var(--kf-focus-ring,3px solid var(--kf-accent,#7cff2b));outline-offset:-2px}img{width:24px;height:24px;object-fit:contain}span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}`;
...
const HEADER_BUTTON_BASE_CSS = `button{box-sizing:border-box;display:inline-flex;height:36px;align-items:center;justify-content:center;gap:7px;padding:0 11px;border:1px solid rgba(var(--kf-accent-rgb,124,255,43),var(--kf-header-edge-alpha,.38));border-radius:var(--kf-radius,8px);background:linear-gradient(180deg,rgba(var(--kf-accent-rgb,124,255,43),.12),rgba(var(--kf-accent-rgb,124,255,43),.055));color:var(--kf-text,#f5f8f6);box-shadow:inset 0 1px 0 rgba(255,255,255,.035);cursor:pointer;font:750 12px/1 Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;letter-spacing:.015em;white-space:nowrap;transition:border-color 120ms ease,background 120ms ease,color 120ms ease,transform 80ms ease}button:hover{border-color:var(--kf-accent,#7cff2b);background:rgba(var(--kf-accent-rgb,124,255,43),.15);color:var(--kf-accent,#7cff2b)}button:active{transform:scale(.97)}button:focus-visible{outline:var(--kf-focus-ring,3px solid var(--kf-accent,#7cff2b));outline-offset:var(--kf-focus-offset,2px)}@media (max-width:960px){button{width:36px;padding:0}span{display:none}}@media (prefers-reduced-motion:reduce){button{transition-duration:.001ms}button:active{transform:none}}`;
const HEADER_CONTROL_CSS = `:host{display:inline-flex;flex:0 0 auto;gap:6px;color-scheme:dark}*{box-sizing:border-box}${HEADER_BUTTON_BASE_CSS}:host([data-kf-header-actions="essential"]) :is([data-kf-header-multi],[data-kf-header-add-multi]){display:none!important}[data-kf-earned="reward-ready"]{position:relative}[data-kf-earned="reward-ready"]::after{content:'';position:absolute;top:3px;right:3px;width:7px;height:7px;border-radius:4px;background:var(--kf-accent,#7cff2b);animation:kf-earned-pulse 2.4s ease-in-out infinite}@keyframes kf-earned-pulse{0%,100%{opacity:1}50%{opacity:.45}}@media (prefers-reduced-motion:reduce){[data-kf-earned="reward-ready"]::after{animation:none}}img{display:block;width:18px;height:18px;object-fit:contain}.kf-header-multi svg{width:15px;height:15px;fill:currentColor;opacity:.9}.kf-header-add [data-kf-header-add-icon]{font-weight:800;font-size:14px}.kf-header-add[data-in-multi="true"]{border-color:var(--kf-accent,#7cff2b);background:rgba(var(--kf-accent-rgb,124,255,43),.2);color:var(--kf-accent,#7cff2b)}`;
const PROFILE_STATS_CSS = `:host{display:inline-flex}${HEADER_BUTTON_BASE_CSS}svg{width:18px;height:18px;fill:none;stroke:currentColor;stroke-width:2;stroke-linecap:round;stroke-linejoin:round}`;
