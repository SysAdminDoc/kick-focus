...
- Searching the Removed view now offers Restore shown, so the handful you were looking for come back together. Before this the only choices were one at a time or all of them, and all of them is not what a filtered list is asking for.
...
- A configurable emote dock now lives beside the chat composer. It can show Mixed, Favorites, or Recent emotes in Compact or Standard form, with a limit from 4 through every match.
- The dock's library control opens the full emote collection and returns focus to the draft. Dock clicks insert a plain emote name at the caret and never send the message.
- Removed now has a real recovery view. Hidden records keep their artwork, provenance, and usage history until they are restored.

### Changed

- The in-player organizer uses a stable detail tray instead of a floating management card. Favorite, group assignment, and Remove stay inside the chat rail without covering the selected artwork.
- Library search now covers emote names, sources, and custom groups. Groups can move earlier or later, and they can be deleted directly from the organizer.
- Chat emote settings now describe the dock by where it appears and what it contains. Mixed is the default source for new profiles.

### Fixed

- Emote organization no longer loses removed records, hides the only search field, or leaves the user stranded in Settings after opening the library from chat.
- The dock replaces Kick's quick-emote holder cleanly, keeps its controls inside the composer, restores focus, and preserves a zero-send boundary for every insertion path.
- Home-page category cards no longer satisfy the channel-card compatibility probe after Kick's latest shell change.
...
- Optional composer recall now uses a visible Recall button beside chat. It stays disabled until this tab has a public send, cycles the same private five-message ring, and leaves every keyboard gesture with Kick.
...
- The popped-out multi-stream chat window paints the right colours before the page has finished loading. It is a separate window with its own copy of the palette to fall back on, and every colour in that copy was from a palette this build stopped shipping some time ago.
...
- All six copies of the blocklist URL rule now answer the same. Two of them used to accept a URL carrying a username and password, which on a userscript-only install meant those credentials went out to that host on every refresh. Credentials are refused everywhere and the fragment is dropped. If you had such an address saved, the field clears and the subscription switches off rather than sitting on a permanent error.
- A page on one Kick tab can no longer reset another Kick tab's blocked count or clear its badge in the companion.
...
- **The live gate now checks that Kick's own endpoints still exist.** Kick removes endpoints without notice, it deleted the one serving video metadata outright in July 2026, which broke another Kick extension on the spot. Drift detection covered Kick's page markup and said nothing about its API, so a removed endpoint would have degraded into a diagnostics counter nobody reads. Each endpoint this build depends on is now read on every live run, and one that is gone fails the gate by name. An endpoint that answers only for a signed-in session is reported rather than failed, because the gate runs logged out. The list is derived from the module that owns the endpoints, so a new one cannot be added without either a check or a stated reason.
...
- **A catalog emote marked "available" was filed as subscriber-only.** The library merge read the native picker's `available` flag but not the catalog's own `access`, so everything the catalog reported as usable, every Global and Emoji emote, and now everything the account owns, fell through to the locked branch.
...
- **Claim Kick's daily reward automatically** (off by default). When a reward is waiting, Kick Focus opens Kick's own reward dialog and clicks its claim button for you, then closes it again and gives you focus back. It clicks nothing else. A reward without enough watch time shows a disabled button, and this leaves it alone rather than trying, so it can never claim something the account has not earned. It waits until you are not typing and no Kick Focus panel is open, checks at most once every ten minutes, and stops for the day once it claims. The check is shared across your tabs, so four open Kick tabs do not each open the dialog. Signed-in only: the reward button does not exist otherwise.
