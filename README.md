...
[![Version](https://img.shields.io/badge/version-1.29.0-53fc18?style=flat-square)](CHANGELOG.md)
...
- Adds Standard, Theater, and Focus stream layouts, plus Right, Docked, and Hidden chat. Theater keeps the player/chat split inside the viewport, and Kick's chat separator remains draggable with widths saved from 320–520 px.
...
- Isolated companion proof: Chromium 151, logged out, headed and off-screen — 87/87 live checks pass at 1440×900, repeated at 1920×1080 by `release:check` (2026-08-19). The matrix now includes the bounded Theater player/chat split, a real separator drag across both chat layers, and a per-route compatibility sweep that opens Home, Browse, Categories, Search and a channel in turn and compares each against the probe recorded for it, and a 300-message chat burst with every comfort switch on (19 ms per apply cycle in the run above). Checks whose subject Kick did not render on the route being tested are reported as skips that name what was missing, and counted apart from the total; `release:check` refuses to let this line advertise a clean sweep when the run was not one.
