...
[![Version](https://img.shields.io/badge/version-1.3.0-53fc18?style=flat-square)](CHANGELOG.md)
...
- Remembers volume, mute state, quality, and finite VOD position locally with separate privacy toggles; adds favorite and not-interested card actions, configurable Following/Recommended rails, search result counts, and mini-player collision recovery.
...
- Stores settings locally in the userscript manager. There is no analytics, network update code, `@require`, or remote executable code. An optional, off-by-default subscription accepts only user-supplied JSON data containing channels, categories, and keywords.
