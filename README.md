...
[![Version](https://img.shields.io/badge/version-1.39.0-53fc18?style=flat-square)](CHANGELOG.md)
...
- Chat can sit on either side of the player, float as a dock, or stay hidden. Its separator remains adjustable from 320 to 520 px in both directions, including Theater mode.
...
- Optional composer recall keeps this tab's last five public sends in memory. Shift+Up cycles them without taking over ordinary Arrow Up, and nothing from the ring is written to storage.
...
- **The profile comment box has a complete emote workspace.** Kick's search stays at the top, followed by Favorites, Recent, All, custom Groups, and Kick's original groups. Groups are created and renamed in place, and deletion offers Undo without leaving chat. Organize mode selects one emote or every visible result, then moves or removes the selection with its own recovery action. Favorites can be reordered here too. A normal emote click only inserts its name into the comment box and never sends it. Helpful empty and no-result states keep every path recoverable, and only tiles near the viewport enter the page, so a library at the 2400 cap still costs one small window of nodes.
...
- Isolated companion proof: Chromium 151, logged out, headed and off-screen, 96/96 live checks pass with 15 skips at 1440×900 (2026-08-25, against this exact build). The matrix includes the bounded Theater player/chat split, a real separator drag across both chat layers, a per-route compatibility sweep, and a 300-message chat burst with every comfort switch on. It reads Kick's Content-Security-Policy from the home and channel responses, scrolls a live transcript back for five seconds to prove the same message stays in place, checks the Help control on every settings page at 680px, and measures readable contrast in OLED and Slate across settings, confirmation dialogs, toasts, and the companion popup. Checks whose subject Kick did not render are reported as skips that name what was missing and are counted apart from the total. A skip names the subject Kick did not render, most often a surface that needs a signed-in session, and skips are counted apart from the total. The 92-check predecessor also passed at 1920×1080 on 2026-08-19.
- Firefox companion proof: Firefox 153, logged out and headless, the Manifest V2 package is installed as a temporary add-on over WebDriver BiDi and asserted against live Kick by `npm run verify:firefox`. The current run passes 8/8 with one documented popup-navigation skip (2026-08-21).
