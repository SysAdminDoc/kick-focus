...
[![Version](https://img.shields.io/badge/version-1.33.0-53fc18?style=flat-square)](CHANGELOG.md)
...
- **A Viewer page gathers what Kick already tells your account.** Daily reward, channel points, collectibles, Drops, level and streak, each with the source it was read from and how old that reading is. It reads and nothing else. A card that could not read its value says so in a sentence rather than showing a zero, because an empty balance and an unreadable one are not the same thing, and the cards fail one at a time so a refused read costs one card instead of the page. Nothing is requested while the page is closed.
