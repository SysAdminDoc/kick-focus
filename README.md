...
[![Version](https://img.shields.io/badge/version-1.37.0-53fc18?style=flat-square)](CHANGELOG.md)
...
- **Emotes have their own workspace.** The picker opens it directly. Search and filters sit beside a visible group panel, while Select, Select shown, Clear, Move, and Remove make large libraries manageable. A new group becomes the active batch target. Names edit inline, deletion is immediate, and Removed offers a clear restore path. Public artwork from any named channel can still be added without treating it as account access.

![Batch emote grouping in the dedicated library](design/screenshots/emote-library.png)

- **The emote picker is searchable and fast at any size.** Kick's search stays at the top of every custom view. Empty favorites use one compact line, while a populated quick shelf holds up to three rows. Most used and Recent appear when editing favorites, using local send history from the current channel first. Only tiles near the viewport enter the page, so a library at the 2400 cap still costs one small window of nodes.

![Searchable emote shelf](design/screenshots/emote-picker.png)
