...
[![Version](https://img.shields.io/badge/version-1.45.0-53fc18?style=flat-square)](CHANGELOG.md)
...
![Compact channel layout with both rails tucked away](design/screenshots/compact-channel-layout.png)

![Layout controls for rails, chat, density, and header](design/screenshots/settings-layout.png)
...
- **Emotes have their own full library.** The picker opens it directly for deeper review. Search and filters sit beside a permanent group panel, while multi-select, Move, Remove, and Removed recovery keep large collections manageable. Public artwork from a named channel can still be added without treating it as account access.

![Batch emote grouping in the dedicated library](design/screenshots/emote-library.png)

- **The profile comment box has a complete emote workspace.** Kick's search stays at the top, followed by Favorites, Recent, All, custom Groups, and Kick's original groups. Compact fits eight 40-pixel tiles across a 340-pixel chat rail. Balanced and Roomy remain available, with independent Short, Medium, and Tall shelf heights. The native quick strip can be hidden, reduced to 24-pixel controls, or left at Kick's size, with 4, 6, 8, 10, or every saved choice shown. Groups are created and renamed in place, and deletion offers Undo without leaving chat. Organize mode selects one emote or every visible result, then moves or removes the selection with its own recovery action. Hovering or focusing a tile opens Favorite, source channel, and Remove beside it, so no management control covers the artwork at rest. Dragging reorders All, Favorites, or a custom group, while the Earlier and Later controls remain available in Organize mode. Collectible rarity is a single colored letter inside the tile in every density, with the full source detail in its title. Expired subscriber emotes disappear from active views and remain dimmed in Locked, where the source link gives a direct route to resubscribe. Larger pointer targets grow the action rows without changing the scroll window. A normal emote click only inserts its name into the comment box and never sends it. Helpful empty and no-result states keep every path recoverable, and only tiles near the viewport enter the page, so a library at the 2400 cap still costs one small window of nodes.

![Profile comment-box emote workspace](design/screenshots/emote-picker.png)

![Compact emote and quick-strip settings](design/screenshots/settings-content-emotes.png)
...
![Daily reward ready, opening, roulette, and confirmed states](design/qa/comparison-daily-reward-lifecycle-v1.45.png)
...
![Kick Focus multi-stream viewing board](design/screenshots/multistream-board.png)
...
- The auto-hide sidebar concept comes from the "KICK Dropdown" userstyle by IamKoeda ([userstyles.world/style/29036](https://userstyles.world/style/29036), MIT), rebuilt here on this project's own design tokens.
...
- Isolated companion proof: Chromium 151, logged out, headed and off-screen, 101/101 live checks pass with 15 skips at 1440×900 (2026-08-27, against this exact build). The matrix includes the bounded Theater player/chat split, a real separator drag across both chat layers, a per-route compatibility sweep, and a 300-message chat burst with every comfort switch on. It reads Kick's Content-Security-Policy from the home and channel responses, scrolls a live transcript back for five seconds to prove the same message stays in place, checks the Help control on every settings page at 680px, and measures readable contrast in OLED and Slate across settings, the command menu, toasts, and the companion popup. Checks whose subject Kick did not render are reported as skips that name what was missing and are counted apart from the total. The same suite passes 101/101 with 15 skips at 1920×1080 as well.
...
The build concatenates the metadata block and the eight source modules into `dist/kick-focus.user.js` (see [Repository map](#repository-map)). It strips comments and blank code lines, plus indentation outside template content, then emits the same page-world bundle into both companion packages. The extension network rules are generated from the same host lists the page-realm classifier uses, so the layers cannot drift apart; `npm run verify` fails if they do.
...
The build concatenates `core.mjs` → `api.mjs` → `compatibility.mjs` → `storage.mjs` → `live.mjs` → `multistream.mjs` → `settings.mjs` → `runtime.js` into one IIFE, in that order, stripping `import`/`export` as it goes: concat order is what supplies an imported name, so each module can declare its real dependencies and still load on its own under `node --test`.
