...
- Adds a chat pause that arms when you scroll the transcript up, per-channel keyword highlights and private notes, optional playback diagnostics, and a panic switch that restores Kick's native page without a reload.
...
- Isolated companion proof: Chromium 151, logged out, headed and off-screen, 91/91 live checks pass at 1440×900, repeated at 1920×1080 by `release:check` (2026-08-19). The matrix now includes the bounded Theater player/chat split, a real separator drag across both chat layers, and a per-route compatibility sweep that opens Home, Browse, Categories, Search and a channel in turn and compares each against the probe recorded for it, and a 300-message chat burst with every comfort switch on (19 ms per apply cycle in the run above). It also reads Kick's Content-Security-Policy off the home and channel document responses and fails if one appears that would refuse the inline injection the Firefox companion depends on, scrolls a live transcript back to prove chat pause arms and Resume leaves it, and checks that the same Help control sits in the same place on every settings page at 680px. Checks whose subject Kick did not render on the route being tested are reported as skips that name what was missing, and counted apart from the total; `release:check` refuses to let this line advertise a clean sweep when the run was not one.
