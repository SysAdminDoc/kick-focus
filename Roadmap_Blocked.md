...
- P2 — R-24, establish a remote + stable HTTPS URL so the userscript can auto-update. Blocked on an operator decision, not on work: the checkout has no git remote, so there is no `@downloadURL`/`@updateURL` to point a manager at, and wiring one is publication — which ROADMAP defers until explicitly approved. On approval: raw HTTPS URL into `src/metadata.txt`, plus a sha256 `update_hash` in the Firefox `updates.json`. (Moved from ROADMAP 2026-08-16.)
